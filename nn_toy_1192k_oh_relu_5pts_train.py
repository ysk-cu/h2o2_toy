...
# coding: utf-8
#
# nn_toy_1192k_oh_relu_5pts_train.py
...
#   2. [OH]_peak   (argmax OH value)     -> constrains k2/k1 ratio
#   3. t_peak      (argmax OH time)      -> independent timing constraint (HIDDEN_DIM=32)
#   4. OH @ 2*t_peak                     -> constrains k2 (removal, R26)
#   5. OH @ 5*t_peak                     -> constrains k2 at later times
#
...
# NOTE 3 (t_peak): t_peak is extracted dynamically per sample (argmax of the coarsened
#   profile). HIDDEN_DIM=32 is required; 16 units gave 21% error on this probe.
...
LN_F        = 10        # A = A0 * exp(x * LN_F);  see NOTE 1
...
HIDDEN_DIM     = 32    # 32 required for t_peak probe (16 gave 21% error)
lr_init        = 0.03
TRAIN_FRAC     = 0.80
VAL_FRAC       = 0.10
EPOCHS         = 5000
...
CHECKPOINT_PATH = 'ckpt_1192k_oh_train_5pts.pt'
RESULT_PATH     = 'result_1192k_oh_relu_5pts_train.pt'
SIGMA_REQS      = {0.1: (0.01, 0.02), 0.3: (0.02, 0.05), 0.5: (0.03, 0.10)}
LOG_EPS         = 1e-12

TARGET_LABELS = ['rise (50% up)', '[OH]_peak', 't_peak', 'OH @ 2*t_peak', 'OH @ 5*t_peak']
...
def find_oh_targets(profile, t_sim):
    """Return 5 nominal reference times for the target probes.
    P3 (t_peak) stores the nominal peak time; actual t_peak is extracted
    dynamically per sample in run_single."""
    ...
    times = np.array([
        t_sim[i1],    # P1: rise (50%)
        t_peak,       # P2: [OH]_peak
        t_peak,       # P3: t_peak reference (nominal; actual extracted per sample)
        2.0 * t_peak, # P4: OH @ 2*t_peak
        5.0 * t_peak, # P5: OH @ 5*t_peak
    ])
...
# Coarsened training grid: 1 us steps, 1000 pts, covers 1 ms.
# run_single runs the full profile to find t_peak dynamically per sample.
_TRAIN_STRIDE = 10
T_TRAIN       = T_FINE[::_TRAIN_STRIDE]     # shape (1000,)
N_TRAIN       = len(T_TRAIN)
_i_rise_nom   = int(np.argmin(np.abs(T_TRAIN - TARGET_TIMES[0])))

print('\nSelected OH target probes (nominal):')
_nom_vals = [
    np.interp(TARGET_TIMES[0], T_FINE, _nom) * 1e6,   # rise OH [ppm]
    _nom[_i_pk] * 1e6,                                 # [OH]_peak [ppm]
    T_FINE[_i_pk] * 1e6,                               # t_peak [us]
    np.interp(TARGET_TIMES[3], T_FINE, _nom) * 1e6,   # OH @ 2*t_peak [ppm]
    np.interp(TARGET_TIMES[4], T_FINE, _nom) * 1e6,   # OH @ 5*t_peak [ppm]
]
_units = ['ppm', 'ppm', 'us', 'ppm', 'ppm']
...
# -- Cantera simulation -- 5 target probes (rise/OH_peak/t_peak/2*t_peak/5*t_peak) --
...
def run_single(x_vec):
    try:
        ...
        # Run coarsened profile to find t_peak dynamically
        profile = np.empty(N_TRAIN)
        for k in range(N_TRAIN):
            net.advance(T_TRAIN[k])
            profile[k] = reactor.thermo.X[oh_idx]

        i_peak        = int(np.argmax(profile))
        t_peak_actual = float(T_TRAIN[i_peak])

        y_out = np.array([
            profile[_i_rise_nom],                                          # P1: OH @ rise
            profile[i_peak],                                               # P2: [OH]_peak
            t_peak_actual,                                                 # P3: t_peak (s)
            float(np.interp(2.0 * t_peak_actual, T_TRAIN, profile)),      # P4: OH @ 2*t_peak
            float(np.interp(5.0 * t_peak_actual, T_TRAIN, profile)),      # P5: OH @ 5*t_peak
        ])
        return True, y_out
    except Exception:
        return False, None
...
# -- Architecture -- ReLU, 4 inputs -> N_TARGETS outputs -----------------------
...
print('\nTraining (1192 K OH, 5-pt: rise/OH_peak/t_peak/2*t_peak/5*t_peak, ReLU, hidden=32) ...')
t0 = time.time()
...
print(f'\n{"="*72}')
print('  1192 K OH -- 5-pt (rise/OH_peak/t_peak/2*t_peak/5*t_peak)  |  Zhang Table 1')
print(f'{"="*72}')
all_pass = True
