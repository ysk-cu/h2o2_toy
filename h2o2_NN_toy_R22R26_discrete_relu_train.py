...
# coding: utf-8
#
# Discrete-target NN surrogate — ReLU, R22 + R26 (4 params)
#
# Parameters:
#   x[0] = lnA_R22   (R22 fall-off low-rate A)
#   x[1] = Ea_R22    (R22 fall-off low-rate Ea)
#   x[2] = lnA_R26   (R26 simple Arrhenius A)
#   x[3] = Ea_R26    (R26 simple Arrhenius Ea)
#
# R22: H2O2(+M) <=> OH + OH (+M)  — falloff, index 21 (0-based)
# R26: H2O2 + OH <=> HO2 + H2O   — simple Arrhenius, index 25 (0-based)
...
print(f'R22 nominal: A={NOMINAL_A_R22:.3e}  Ea={NOMINAL_EA_R22_cal:.0f} cal/mol')
print(f'R26 nominal: A={NOMINAL_A_R26:.3e}  Ea={NOMINAL_EA_R26_cal:.0f} cal/mol')
...
CHECKPOINT_PATH = 'simulation_checkpoint_r22r26_discrete.pt'
RESULT_PATH     = 'relu_r22r26_discrete_targets.pt'
SIGMA_REQS      = {0.1: (0.01, 0.02), 0.3: (0.02, 0.05), 0.5: (0.03, 0.10)}
LOG_EPS         = 1e-10


# ── Auto-select TARGET_TIMES from nominal profile (x=0) ──────────────────────

def run_full_profile_nominal():
...
X_samples, L_samples = multiscale_sobol(TOTAL_SAMPLES, SIGMA_LIST, RATIO_LIST, d=INPUT_DIM)


# ── Cantera simulation (perturb R22 and R26) ──────────────────────────────────
...
if os.path.exists(CHECKPOINT_PATH):
    ...
    print(f'Resuming from idx {start_idx}/{TOTAL_SAMPLES}: '
          f'{len(y_list)} valid entries kept, {dropped} dropped, {n_failed} failed')
else:
...
for i in range(start_idx, TOTAL_SAMPLES):
    ...
    if (i + 1) % 2000 == 0:
        ...
        rate    = (i + 1 - start_idx) / max(elapsed, 1e-9)
...
print('\nTraining: discrete-target ReLU surrogate (R22 + R26, 4 params)')
t0 = time.time()
...
print(f'\n{"="*70}')
print('  ReLU discrete-target (R22+R26) — Zhang Table 1')
print(f'{"="*70}')
all_pass = True
