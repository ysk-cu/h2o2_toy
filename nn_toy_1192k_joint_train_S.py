...
# coding: utf-8
#
# nn_toy_1192k_joint_train_S.py
# Joint OH + H2O surrogate with JOINT D-optimal target selection (1192 K)
# Mixture: 2216 ppm H2O2 / 1364 ppm H2O / 682 ppm O2 / Ar
#
# KEY DIFFERENCE: Targets are selected for BOTH species jointly, not independently.
# This ensures targets are compatible when doing joint inference.
#
...
CHECKPOINT_PATH = 'ckpt_1192k_joint_train_S.pt'
RESULT_PATH     = 'result_1192k_joint_train_S.pt'
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30):
    """
    Joint D-optimal target selection for both species.
    Selects times that maximize det(S_joint^T S_joint) where S_joint stacks both sensitivities.
    """
    cand_oh  = np.where(_nom_oh[np.searchsorted(T_FINE, t_oh)] * 1e6 >= oh_min_ppm)[0] if len(t_oh) > 0 else np.arange(len(t_oh))
    cand_h2o = np.arange(len(t_h2o))
...
OH_LABELS  = ['rise (50% up)', 'peak', 'mid-decay (33% down)']
H2O_LABELS = ['H2O early', 'H2O mid', 'H2O late']

print('\nOH target probes (joint D-optimal):')
...
print('\nH2O target probes (joint D-optimal):')
...
print(f'\nTraining (joint OH+H2O, JOINT D-optimal targets, '
      f'hidden={HIDDEN_DIM}, w_OH={WEIGHT_OH}, w_H2O={WEIGHT_H2O}, ReLU) ...')
t0 = time.time()
...
pass_oh  = zhang_table('1192 K OH  (joint NN, joint D-opt)', rel_err_oh)
pass_h2o = zhang_table('1192 K H2O (joint NN, joint D-opt)', rel_err_h2o)
