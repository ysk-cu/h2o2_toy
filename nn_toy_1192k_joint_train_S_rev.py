...
# coding: utf-8
#
# nn_toy_1192k_joint_train_S_rev.py
# Joint OH + H2O surrogate with JOINT D-optimal target selection and time-window constraints (1192 K)
# Mixture: 2216 ppm H2O2 / 1364 ppm H2O / 682 ppm O2 / Ar
#
# KEY DIFFERENCES FROM _S:
#   1. Targets are selected for BOTH species jointly (not independently)
...
# OH and H2O carry their k1/k2 information in DIFFERENT time regions, so a single
# shared window starves one species. Using one global [0.1us, 0.1ms] window pushed
# k1 to be set almost entirely by OH (peak overshoot) because H2O lost its late-time
# rise/plateau anchor -- that anchor is what pulls k1 back down via H2O2->H2O yield.
#
# OH: tight early window -- peak/early-decay carries k1 and k1/k2-ratio information.
#   Late OH decay (>0.1 ms) has ~34% structural overshoot from missing radical-pool
#   chemistry (OH+HO2 cycling), so it's excluded to keep k2 from absorbing that error.
T_MIN_TARGET_OH = 1e-7    # 0.1 µs
T_MAX_TARGET_OH = 1e-4    # 0.1 ms

# H2O: wider window -- H2O's k1 information lives in the rise-to-plateau, which
# extends past 0.1 ms. Unlike the OH tail, the H2O plateau is a yield constraint
# (how much H2O2 converts), not a rate fit, so it doesn't force k2 inflation the
# way the OH late-decay tail does. Matches nn_toy_1192k_h2o_train_S_rev.py.
T_MIN_TARGET_H2O = 1e-5    # 10 µs
T_MAX_TARGET_H2O = 4e-4    # 0.4 ms

# ── NN / training hyper-parameters ───────────────────────────────────────────

HIDDEN_DIM     = 32            # larger for joint training
lr_init        = 0.03
TRAIN_FRAC     = 0.80
VAL_FRAC       = 0.10
EPOCHS         = 5000
...
CHECKPOINT_PATH = 'ckpt_1192k_joint_train_S_rev.pt'
RESULT_PATH     = 'result_1192k_joint_train_S_rev.pt'
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30,
    ...
    """
    Joint D-optimal target selection for both species with species-specific
    time-window constraints (OH's information is early, H2O's extends later).
    Selects times that maximize det(S_joint^T S_joint) where S_joint stacks both sensitivities.

    Args:
        t_min_oh, t_max_oh   : time window for OH candidate selection
        t_min_h2o, t_max_h2o : time window for H2O candidate selection
    """
    # Candidates: within trustworthy time window and above OH measurability threshold
    ...
    cand_h2o_window = np.where((t_h2o >= t_min_h2o) & (t_h2o <= t_max_h2o))[0]
    cand_h2o = cand_h2o_window

    chosen_oh  = []
    chosen_h2o = []

    # Seed: strongest combined sensitivity in each species
    norm_oh  = np.linalg.norm(S_oh_mat[cand_oh],  axis=1) if len(cand_oh) > 0 else np.array([])
    norm_h2o = np.linalg.norm(S_h2o_mat[cand_h2o], axis=1)

    if len(norm_oh) > 0:
        chosen_oh.append(cand_oh[np.argmax(norm_oh)])
    chosen_h2o.append(cand_h2o[np.argmax(norm_h2o)])

    # Greedily add targets maximizing joint det
    for iteration in range(max(n_oh, n_h2o) - 1):
        best_det = -np.inf
        best_add_oh = None
        best_add_h2o = None

        # Try adding each candidate from OH
        if len(chosen_oh) < n_oh:
            for c in cand_oh:
                if c not in chosen_oh:
                    S_test = S_oh_mat[chosen_oh + [c]]
                    if S_test.shape[0] >= 2:
        ...
        # Try adding each candidate from H2O
        if len(chosen_h2o) < n_h2o:
            for c in cand_h2o:
                if c not in chosen_h2o:
                    S_test = S_h2o_mat[chosen_h2o + [c]]
                    if S_test.shape[0] >= 2:
                        det_val = np.linalg.det(S_test.T @ S_test)
                        if det_val > best_det:
                            best_det = det_val
        ...
        if best_add_oh is not None:
            chosen_oh.append(best_add_oh)
        if best_add_h2o is not None:
            chosen_h2o.append(best_add_h2o)

        if best_add_oh is None and best_add_h2o is None:
            break
...
print(f'\nOH target probes (window {T_MIN_TARGET_OH*1e6:.1f} µs .. {T_MAX_TARGET_OH*1e3:.2f} ms, joint D-optimal):')
...
print(f'\nH2O target probes (window {T_MIN_TARGET_H2O*1e6:.1f} µs .. {T_MAX_TARGET_H2O*1e3:.2f} ms, joint D-optimal):')
...
print('\nTraining (1192 K joint OH+H2O, 3-pt each, OH: 0.1us-0.1ms / H2O: 10us-0.4ms) ...')
t0 = time.time()
...
print(f'\n{"="*72}')
print('  1192 K Joint (OH + H2O) — 3-pt each (constrained)  |  Zhang Table 1')
print(f'{"="*72}')
