...
CHECKPOINT_PATH = 'ckpt_1192k_oh_train_4pts.pt'
RESULT_PATH     = 'result_1192k_oh_relu_4pts_train.pt'
SIGMA_REQS      = {0.1: (0.01, 0.02), 0.3: (0.02, 0.05), 0.5: (0.03, 0.10)}
LOG_EPS         = 1e-12

TARGET_LABELS = ['rise (50% up)', '[OH]_peak', 'OH @ 2·t_peak', 'OH @ 5·t_peak']
...
def find_oh_targets(profile, t_sim):
    """Return 4 fixed nominal times for the target probes."""
...
# ── Cantera simulation — 4 target probes (rise/OH_peak/2×t_peak/5×t_peak) ──────
...
print('\nTraining (1192 K OH, 4-pt: rise/OH_peak/2×t_peak/5×t_peak, ReLU) ...')
t0 = time.time()
...
print(f'\n{"="*72}')
print('  1192 K OH — 4-pt (rise/OH_peak/2×t_peak/5×t_peak)  |  Zhang Table 1')
print(f'{"="*72}')
all_pass = True
