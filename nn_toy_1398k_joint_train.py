...
# coding: utf-8
#
# nn_toy_1398k_joint_train.py
# Joint OH + H2O surrogate with JOINT D-optimal target selection and time-window constraints (1398 K)
...
T_MAX_TARGET_H2O = 1e-4    # 0.1 ms — MUST NOT exceed the experimental data range.
                           # Was 4e-4 (0.4 ms), copied from the 1192 K file whose
                           # data ran to ~1 ms; at 1398 K that would have selected
                           # H2O targets past the real data, silently corrupted by
                           # np.interp's clip-to-boundary behavior (see above).
...
CHECKPOINT_PATH = 'ckpt_1398k_joint_train.pt'
RESULT_PATH     = 'result_1398k_joint_train.pt'
...
print('\nTraining (1398 K joint OH+H2O, 3-pt each, OH: 10us-0.1ms / H2O: 10us-0.1ms) ...')
t0 = time.time()
