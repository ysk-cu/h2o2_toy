...
# coding: utf-8
#
# nn_toy_1398k_joint_train_k5.py
# 5-OH-TARGET variant: N_OH_TARGETS=5 (was 3) to dilute the 1us rise's k1 pull
# and add decay-region k5 constraint. Base = normalized k5.py (50% ceiling). Watch
# fittability: 8 outputs on 16 nodes is a stretch (3-OH already failed Zhang).
# Joint OH + H2O surrogate (single NN)
# INDEPENDENT per-species D-optimal target selection
# CONDITION-ADAPTIVE time windows, but OH's window is the FULL data range
...
PARAM_NAMES = ['lnA_R1', 'Ea_R1', 'lnA_R2', 'Ea_R2', 'lnA_R5', 'Ea_R5']
INPUT_DIM   = 6
...
# T_MAX_TARGET_OH is NOT a fixed constant -- it's a fraction-of-peak ceiling,
# RE-INTRODUCED (the first k5 run used the full range and FAILED) but tuned for k5.
# All three regimes verified by direct det/cond on the OH k1/k2/k5 Gram matrix:
#   - 71.5%-of-peak (the old 2-param structural-error ceiling): cut OH at ~25us,
#     excluding ALL k5 signal -> k5 unidentifiable. TOO aggressive.
#   - full data range (first k5 run): decay target at 32% of peak (164 ppm), which
#     is un-fittable -- OH FAILED all Zhang noise sets (mean 1.2-4.8%) because OH is
#     small there (relative-error amplified) and k5's [OH]^2 removal makes it very
#     nonlinear. TOO deep.
#   - ~50%-of-peak (this value): decay target lands ~49us at OH ~261 ppm -- clearly
#     fittable (comparable to the 1us rise target that always fit) -- while k5 stays
#     well-conditioned (Gram cond ~44, vs 25 at full range, 210 at the 70% ceiling).
#     Captures ~80% of |S_k5|'s peak magnitude. THE SWEET SPOT.
...
T_MIN_TARGET_H2O = 1e-6    # 1 µs -- same reasoning as OH's floor

T_MAX_H2O_RISE_FRAC = 0.95

# ── NN / training hyper-parameters ───────────────────────────────────────────

HIDDEN_DIM     = 16            # bumped from 16 -- 6 inputs / 8 outputs now (was 4/7)
lr_init        = 0.03
TRAIN_FRAC     = 0.80
VAL_FRAC       = 0.10
EPOCHS         = 5000
...
CHECKPOINT_PATH = 'ckpt_1398k_joint_k5_5oh.pt'
RESULT_PATH     = 'result_1398k_joint_k5_5oh.pt'
...
# ── OH FRACTION-OF-PEAK CEILING (k5-tuned; see long note above) ───────────────
# Cap OH targets at the time OH decays to T_MAX_OH_PEAK_FRAC of its own peak, but
# never past the hard data ceiling. Keeps the k5-informative decay in-window while
# staying shallow enough for the surrogate to fit.
_oh_dec   = np.where(_nom_oh[_i_pk:] <= T_MAX_OH_PEAK_FRAC * _oh_peak)[0]
_t_oh_cut = float(T_FINE[_i_pk + _oh_dec[0]]) if len(_oh_dec) else float(T_FINE[-1])
T_MAX_TARGET_OH = min(T_MAX_TARGET_OH_DATA_CEILING, _t_oh_cut)
print(f'  OH decays to {T_MAX_OH_PEAK_FRAC*100:.0f}% of peak at t={_t_oh_cut*1e3:.4f} ms; '
      f'ceiling={T_MAX_TARGET_OH_DATA_CEILING*1e3:.4f} ms -> T_MAX_TARGET_OH={T_MAX_TARGET_OH*1e3:.4f} ms')
...
S_oh = np.vstack([S_k1_oh, S_k2_oh, S_k5_oh]).T   # shape: (N_FINE, 3)
...
S_h2o = np.vstack([S_k1_h2o, S_k2_h2o, S_k5_h2o]).T   # shape: (N_STEPS, 3)
...
def _select_independent(S_mat, t_grid, cand, n_targets, dt_min):
    ...
    this species' own Gram matrix. Unchanged from _train4 -- generalizes to any
    number of sensitivity columns with no modification (here: 3, for k1/k2/k5).
    """
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30,
    ...
    """
    INDEPENDENT per-species D-optimal selection (name kept for drop-in
    compatibility with the downstream chronological-advance / run_single code).
    See _train2/_train4 for why independent (not cross-species-competitive)
    selection was adopted. Unchanged here except the Gram matrices are now
    3-column (k1, k2, k5) instead of 2-column.
    """
...
print(f'\nOH target probes (window {T_MIN_TARGET_OH*1e6:.1f} µs .. {T_MAX_TARGET_OH*1e3:.4f} ms, independent D-optimal, k1/k2/k5):')
...
print(f'\nH2O target probes (window {T_MIN_TARGET_H2O*1e6:.1f} µs .. {T_MAX_TARGET_H2O*1e3:.4f} ms, independent D-optimal, k1/k2/k5):')
...
print(f'\nTraining (1398 K joint OH+H2O+k5, {N_OH_TARGETS}-pt OH / {N_H2O_TARGETS}-pt H2O, '
      f'OH: {T_MIN_TARGET_OH*1e6:.0f}us-{T_MAX_TARGET_OH*1e3:.4f}ms [{T_MAX_OH_PEAK_FRAC*100:.0f}%-peak ceiling] / '
...
print(f'\n{"="*72}')
print('  1398 K Joint (OH + H2O), 3 active params (k1,k2,k5)  |  Zhang Table 1')
print(f'{"="*72}')
