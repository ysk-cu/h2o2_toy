...
# coding: utf-8
#
# nn_toy_1192k_h2o_train_S_rev.py  —  REVISED H2O surrogate
# Train H2O surrogate with time-window constraints for joint OH+H2O inference.
#
# KEY CHANGE: Add time-window constraints [T_MIN_TARGET, T_MAX_TARGET] to ensure
# targets are selected only from the trustworthy region. This prevents fitting
# targets in the structural-error tail where the model overshoots due to missing
# radical-pool chemistry.
#
...
# ── Time-window constraints for trustworthy target selection ─────────────────
# T_MIN_TARGET: Early time boundary. Keep well below ~10 µs where instrument
#   response and time-zero artifacts become significant. Set to 100 ns to be safe.
# T_MAX_TARGET: Late time boundary. The nominal model carries ~34% structural
#   overshoot in the late decay due to missing radical-pool chemistry (OH + HO2,
#   radical cycling). Constraining to ~0.1 ms avoids fitting this structural-error
#   region, which forces k2 to absorb error and degrades joint H2O prediction.
T_MIN_TARGET = 1e-5    # 10 µs
T_MAX_TARGET = 4e-4    # 0.4 ms
...
CHECKPOINT_PATH = 'ckpt_1192k_h2o_train_S_rev.pt'
RESULT_PATH     = 'result_1192k_h2o_train_S_rev.pt'
...
# ── H2O target selection with k1+k2 sensitivity and time-window constraints ──
...
def select_h2o_targets(t_grid, H2O_nom, S_k1, S_k2, n_targets, min_dt=0.03e-3,
                       t_min=T_MIN_TARGET, t_max=T_MAX_TARGET):
    """
    Select H2O targets using combined k1+k2 sensitivities within trustworthy time window.
    ...
    Args:
        ...
        t_min, t_max : time window for candidate selection

    Returns sorted indices into t_grid.
    """
    S = np.vstack([S_k1, S_k2]).T  # shape: (N, 2)

    # Candidates: within trustworthy time window
    cand = np.where((t_grid >= t_min) & (t_grid <= t_max))[0]

    if len(cand) == 0:
        raise ValueError(f"No candidates in time window [{t_min*1e6:.1f} µs, {t_max*1e3:.2f} ms]")
...
TARGET_LABELS = [f'H2O D-opt @ {t*1e3:.4f} ms' for t in TARGET_TIMES]

print(f'H2O targets (window {T_MIN_TARGET*1e6:.1f} µs .. {T_MAX_TARGET*1e3:.2f} ms):')
for lbl, t, i in zip(TARGET_LABELS, TARGET_TIMES, idx):
    print(f'  {lbl}   H2O ~ {_h2o_nom[i]*1e6:.1f} ppm   S_k1={S_k1[i]:+.3f}  S_k2={S_k2[i]:+.3f}')
...
for i in range(start_idx, TOTAL_SAMPLES):
    ...
    if (i + 1) % 2000 == 0:
        ...
        torch.save({'last_index': i, 'y_list': y_list, 'x_list': x_list,
                    ...
                    'target_times': TARGET_TIMES, 'target_labels': TARGET_LABELS},
                   CHECKPOINT_PATH)
...
print('\nTraining (1192 K H2O, 3-pt: constrained to 0.1 µs - 0.1 ms) ...')
t0 = time.time()
...
print(f'\n{"="*72}')
print('  1192 K H2O — 3-pt (constrained)  |  Zhang Table 1')
print(f'{"="*72}')
all_pass = True
...
for j, (lbl, t_tgt) in enumerate(zip(TARGET_LABELS, TARGET_TIMES)):
    print(f'  {j+1}. {lbl:<30} t={t_tgt*1e3:.4f} ms   err={rel_err[:, j].mean()*100:.2f}%')
