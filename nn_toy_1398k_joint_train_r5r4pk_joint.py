...
# coding: utf-8
#
# nn_toy_1398k_joint_train_k5r4_joint.py
# Joint OH + H2O surrogate (single NN)
# 1398 K, active reactions k1/k2/k5 + k4 (HO2+OH, Burke R4)
...
CHECKPOINT_PATH = 'ckpt_1398k_joint_r5r4pk_joint_dopt.pt'
RESULT_PATH     = 'result_1398k_joint_r5r4pk_joint_dopt_32.pt'
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30,
    ...
    if len(cand_oh) > 0:
        # PEAK-ANCHOR (Case 2): seed the OH set with the peak time instead of the
        # max-||S|| candidate. The OH peak carries the strongest R1/R2 signal but the
        # pure joint D-optimal only BRACKETS it (1/12/26/66 us) rather than sampling
        # it, so peak fidelity suffers. Anchoring one target on the peak directly
        # constrains it; the greedy loop then fills the remaining 3 OH D-optimally
        # (>= dt_min_oh away). This is the peak-fidelity lever that adding R3 is NOT.
        chosen_oh.append(cand_oh[int(np.argmax(_nom_oh[cand_oh]))])
    if len(cand_h2o) > 0:
...
print(f'\nTraining (1398 K joint OH+H2O+k5+k4, {N_OH_TARGETS}-pt OH / {N_H2O_TARGETS}-pt H2O, '
...
print(f'\n{"="*72}')
print('  1398 K Joint (OH + H2O), 4 active params (k1,k2,k5,k4, TRUE JOINT D-OPT)  |  Zhang Table 1')
print(f'{"="*72}')
