...
CHECKPOINT_PATH = 'ckpt_1192k_oh_train_S_rev2.pt'
RESULT_PATH     = 'result_1192k_oh_train_S_rev2.pt'
SIGMA_REQS      = {0.1: (0.01, 0.02), 0.3: (0.02, 0.05), 0.5: (0.03, 0.10)}
LOG_EPS         = 1e-12

N_TARGETS       = 3  # rise, peak, mid-decay (as per design notes)
# Target labels are now generated from the selected times (the old hardcoded
# rise/peak/mid-decay labels were stale: they described find_oh_targets(),
# which is commented out, not the D-optimal selection actually used).
# Data-quality window for D-optimal candidates:
#   T_MIN_TARGET: exclude the time-zero / instrument-response region. The two
#     Hong OH traces disagree by ~60% at t~0 (31.2 vs 18.7 ppm) and the model
#     has no schlieren/time-zero freedom, so sub-10us "observations" are not
#     trustworthy fitting targets even though model sensitivity peaks there.
#   T_MAX_TARGET: exclude the late-decay tail, where the nominal model carries
...
def select_targets(t_grid, OH_nom, S, n, oh_min_ppm=30, sigma_log=0.05,
    ...
    # D-optimality: maximize det(Sᵀ S) over seected time points
    cand = np.where((OH_nom*1e6 >= oh_min_ppm)
                    & (t_grid >= t_min) & (t_grid <= t_max))[0]
    # measurable AND inside the trustworthy data window
...
TARGET_LABELS = [f'D-opt @ {t*1e3:.4f} ms' for t in TARGET_TIMES]
print('OH D-optimal targets (window %.0f us .. %.2f ms):' % (T_MIN_TARGET*1e6, T_MAX_TARGET*1e3))
for lbl, t in zip(TARGET_LABELS, TARGET_TIMES):
    print(f'  {lbl}   nominal OH ~ {np.interp(t, T_FINE, _nom)*1e6:.1f} ppm')
