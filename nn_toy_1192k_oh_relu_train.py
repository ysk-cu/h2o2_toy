...
HIDDEN_DIM     = 32    # if Set 1 fails, bump to 32 (OH peak is sharp)
lr_init        = 0.03
TRAIN_FRAC     = 0.80
VAL_FRAC       = 0.10
EPOCHS         = 5000
...
RESULT_PATH     = 'result_1192k_oh_relu_train_32.pt'
...
def find_oh_targets(profile, t_sim):
    """rise(50% up) / peak / mid-decay(33% down)."""
    i_peak  = int(np.argmax(profile))
    oh_peak = float(profile[i_peak])
    oh_init = float(profile[0])
    oh_end  = float(profile[-200:].mean())
    rise_gap, decay_gap = oh_peak - oh_init, oh_peak - oh_end

    # P1 — rising edge at 50% of the rise (k1)
    i1 = int(np.argmin(np.abs(profile[:i_peak] - (oh_init + 0.50 * rise_gap))))
    # P2 — peak (k2/k1)
    i2 = i_peak
    # P3 — 33% down the decay (k2)
    post = profile[i_peak:]
    i3 = i_peak + int(np.argmin(np.abs(post - (oh_peak - 0.33 * decay_gap))))

    times = np.array([t_sim[i1], t_sim[i2], t_sim[i3]])
    dt_guard = (t_sim[1] - t_sim[0]) * 2
    for k in range(1, len(times)):           # enforce strictly increasing
...
print('\nSelected OH target times:')
for j, (lbl, t_tgt) in enumerate(zip(TARGET_LABELS, TARGET_TIMES)):
    oh_ppm = np.interp(t_tgt, T_FINE, _nom) * 1e6
    print(f'  {j+1}. {lbl:<22}  t = {t_tgt*1e3:.4f} ms   OH ~ {oh_ppm:.1f} ppm')
...
# ── Cantera simulation — OH at the 4 target times ─────────────────────────────
...
def run_single(x_vec):
    try:
        ...
        new_Ea_R22 = (NOMINAL_EA_R22_cal + x_vec[1] * SIGMA_E) * 4184.0   # cal/mol -> J/kmol
        ...
        for j, t_tgt in enumerate(TARGET_TIMES):      # TARGET_TIMES is increasing
            net.advance(t_tgt)
...
print('\nTraining (1192 K OH, 3-pt: rise/peak/mid-decay, GELU) ...')
t0 = time.time()
