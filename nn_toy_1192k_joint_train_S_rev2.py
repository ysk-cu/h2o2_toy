...
T_MIN_TARGET_OH = 1e-5    # 10 µs — MUST exclude the time-zero/instrument-response
                          # region: the two Hong OH traces disagree ~60% at t~0
                          # (31.2 vs 18.7 ppm) and a target there (2.1 µs) biases
                          # k1 up ~25-30%, which over-speeds the H2O early rise.
...
CHECKPOINT_PATH = 'ckpt_1192k_joint_train_S_rev3.pt'
RESULT_PATH     = 'result_1192k_joint_train_S_rev3.pt'
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30,
                        ...
                        dt_min_oh=5e-6, dt_min_h2o=2e-5):
    """
    ...
    species plus the candidate. This fixes two defects of the previous version:
      (1) it compared per-species dets (H2O's Gram is ~rank-1, det ~ 0, so H2O
          could never win and ended up with a single target -> the remaining
          y_h2o slots were left as uninitialized np.empty garbage);
      (2) the loop budget max(n_oh, n_h2o)-1 could not fill both quotas.
    dt_min_* enforces a minimum time separation between same-species targets so
    rank-1 species (H2O) don't cluster all picks at the sensitivity maximum.
    """
    ...
    if len(cand_oh) > 0:
        chosen_oh.append(cand_oh[np.argmax(np.linalg.norm(S_oh_mat[cand_oh], axis=1))])
    if len(cand_h2o) > 0:
        chosen_h2o.append(cand_h2o[np.argmax(np.linalg.norm(S_h2o_mat[cand_h2o], axis=1))])
    ...
    def _joint_det(extra_oh=None, extra_h2o=None):
        ...
        S = np.vstack([S_oh_mat[sel_oh], S_h2o_mat[sel_h2o]])
