...
# coding: utf-8
#
# optimize_joint_generic_comp.py
...
#
# This "_comp" variant additionally:
#   - prints the optimized rate parameters in physical units (A, Ea) at the MAP
#     estimate, mirroring optimize_1398k_joint_nn_k5.ipynb's cell-7 printout
#   - overlays a reference mechanism (--carly_yaml, fixed/independent rate
#     parameters) on the comparison figure in green, with a +/-carly_pct
#     A-factor envelope drawn as dashed green lines
#
# Usage:
#   python optimize_joint_generic_comp.py --result result_1192k_joint_r5_joint_dopt.pt \
...
import argparse, os, numpy as np, torch, torch.nn as nn, pandas as pd, cantera as ct
import matplotlib
matplotlib.use("Agg")
...
# Known experiment conditions (Hong E2/E1), keyed by the "1192k"/"1398k" tag that
# shows up in both the checkpoint filenames and the hong_<TAG>_{oh,h2o}.csv files.
# Mirrors CONDITIONS in validation.ipynb. Add entries here if new conditions show up.
KNOWN_CONDITIONS = {
    "1192k": dict(T=1192.0, P_atm=1.950, xH2O2=2220e-6, xH2O=1360e-6, xO2=680e-6, csv_tag="1192K"),
    "1398k": dict(T=1398.0, P_atm=1.909, xH2O2=2540e-6, xH2O=1230e-6, xO2=620e-6, csv_tag="1398K"),
}

p = argparse.ArgumentParser()
p.add_argument("--result", required=True,
               help="checkpoint .pt. If its filename contains '1192k'/'1398k', the "
                    "condition, csv paths, --out and --tag are auto-filled from it "
                    "(override any of them explicitly to take precedence).")
p.add_argument("--oh_csv")
p.add_argument("--h2o_csv")
p.add_argument("--T", type=float)
p.add_argument("--P_atm", type=float)
p.add_argument("--xH2O2", type=float)
p.add_argument("--xH2O", type=float)
p.add_argument("--xO2", type=float)
p.add_argument("--out")
p.add_argument("--tag", default=None)
p.add_argument("--yaml", default="chem_cti_toy_model_og.yaml")
p.add_argument("--carly_yaml", default="carly_model_for_yusu.yaml")
p.add_argument("--carly_pct", type=float, default=0.20,
               help="+/- fractional A-factor perturbation applied to every reaction in "
                    "--carly_yaml, giving the dashed-green uncertainty envelope.")
args = p.parse_args()

# ── Auto-fill from the --result filename where possible ───────────────────────
_base = os.path.splitext(os.path.basename(args.result))[0]      # e.g. result_1192k_joint_r5r4unf_joint_dopt_32
_slug = _base[len("result_"):] if _base.startswith("result_") else _base
_cond_key = next((k for k in KNOWN_CONDITIONS if k in _base.lower()), None)
_cond = KNOWN_CONDITIONS.get(_cond_key, {})
for _f in ("T", "P_atm", "xH2O2", "xH2O", "xO2"):
    if getattr(args, _f) is None:
        if _f not in _cond:
            p.error(f"--{_f} not given and could not be inferred from --result "
                     f"filename {args.result!r} (recognized tags: {list(KNOWN_CONDITIONS)})")
        setattr(args, _f, _cond[_f])
if args.oh_csv is None or args.h2o_csv is None:
    if "csv_tag" not in _cond:
        p.error(f"--oh_csv/--h2o_csv not given and could not be inferred from --result "
                 f"filename {args.result!r} (recognized tags: {list(KNOWN_CONDITIONS)})")
    args.oh_csv = args.oh_csv or f"hong_{_cond['csv_tag']}_oh.csv"
    args.h2o_csv = args.h2o_csv or f"hong_{_cond['csv_tag']}_h2o.csv"
if args.out is None:
    args.out = f"fig_{_slug}_comp.png"
if args.tag is None:
    args.tag = _slug.replace("_", " ")
print(f"Auto-filled from {args.result!r}: T={args.T} P_atm={args.P_atm} "
      f"oh_csv={args.oh_csv!r} h2o_csv={args.h2o_csv!r} out={args.out!r}")
...
# ── Optimized rate parameters in physical units (mirrors notebook cell-7) ─────
print("\n" + "="*72)
print(f"  Optimized rate parameters at joint MAP estimate ({'/'.join(r[0] for r in RXN)})")
print("="*72)
for j, (name, idx, fall, A, b, Ea) in enumerate(RXN):
    dlnA, dEa = x_opt[2*j], x_opt[2*j+1]
    sig_lnA, sig_Ea = sig_opt[2*j], sig_opt[2*j+1]
    A_opt = A*np.exp(dlnA*LN_F); Ea_opt = Ea + dEa*SIGMA_E
    print(f"  {name} ({'falloff low-P' if fall else 'Arrhenius'})  "
          f"A : {A:.3e} -> {A_opt:.3e}  (x{np.exp(dlnA*LN_F):.3f})  "
          f"2sig: x{np.exp(2*sig_lnA*LN_F):.2f}")
    print(f"  {name} Ea : {Ea:.0f} -> {Ea_opt:.0f} cal/mol  (shift {dEa*SIGMA_E:+.0f})  "
          f"2sig: +/-{2*sig_Ea*SIGMA_E:.0f} cal/mol")
print("="*72)
...
# ── Carly reference-model overlay: fixed/independent mechanism, run once at its
# own nominal rate parameters. The dashed-green envelope is +/-carly_pct applied
# directly to that nominal H2O/OH mole-fraction profile (not an A-factor bump).
HAVE_CARLY = bool(args.carly_yaml) and os.path.exists(args.carly_yaml)
if HAVE_CARLY:
    def carly_perturb_gas():
        return ct.Solution(args.carly_yaml)
    def carly_run_profiles():
        gas = carly_perturb_gas(); gas.TPX = T_INITIAL, P_INITIAL, INITIAL_X
        rr = ct.IdealGasConstPressureReactor(gas, energy="on"); net = ct.ReactorNet([rr])
        hi_idx = gas.species_index("H2O"); oi_idx = gas.species_index("OH")
        allt = np.concatenate([T_SIM, T_OH]); order = np.argsort(allt, kind="stable")
        hv = np.empty(len(allt)); ov = np.empty(len(allt))
        for k,t in enumerate(allt[order]):
            net.advance(t); hv[order[k]] = rr.thermo.X[hi_idx]; ov[order[k]] = rr.thermo.X[oi_idx]
        return hv[:N_STEPS], ov[N_STEPS:]
    print(f"Carly reference model ({args.carly_yaml}) nominal, +/-{args.carly_pct*100:.0f}% mole-fraction band ...")
    h2o_carly_nom, oh_carly_nom = carly_run_profiles()
    h2o_carly_up, h2o_carly_dn = h2o_carly_nom*(1.0+args.carly_pct), h2o_carly_nom*(1.0-args.carly_pct)
    oh_carly_up,  oh_carly_dn  = oh_carly_nom*(1.0+args.carly_pct),  oh_carly_nom*(1.0-args.carly_pct)
else:
    print(f"Carly reference model skipped (file not found or disabled: {args.carly_yaml!r})")
...
if HAVE_CARLY:
    ax1.plot(T_SIM*1e3,h2o_carly_nom*1e6,"g-",lw=2.0,alpha=0.85,label="Carly model")
    ax1.plot(T_SIM*1e3,h2o_carly_up*1e6,"g--",lw=1.2,alpha=0.8)
    ax1.plot(T_SIM*1e3,h2o_carly_dn*1e6,"g--",lw=1.2,alpha=0.8,label=f"Carly +/-{args.carly_pct*100:.0f}%")
...
if HAVE_CARLY:
    ax2.plot(T_OH*1e3,oh_carly_nom*1e6,"g-",lw=2.0,alpha=0.85,label="Carly model")
    ax2.plot(T_OH*1e3,oh_carly_up*1e6,"g--",lw=1.2,alpha=0.8)
    ax2.plot(T_OH*1e3,oh_carly_dn*1e6,"g--",lw=1.2,alpha=0.8,label=f"Carly +/-{args.carly_pct*100:.0f}%")
