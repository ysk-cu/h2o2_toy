...
# coding: utf-8
#
# nn_toy_1398k_joint_train3.py
# Joint OH + H2O surrogate (single NN, trained on both species together) with
# INDEPENDENT per-species D-optimal target selection and CONDITION-ADAPTIVE
# time-window constraints (1398 K). "Joint" here describes the trained model,
# not the target-selection algorithm -- see select_targets_joint()'s docstring
# for why competing OH and H2O for a shared target quota was dropped in favor
# of each species picking its own best points with the identical selection
# formula.
# Mixture: 22540 ppm H2O2 / 1234 ppm H2O / 617 ppm O2 / Ar
#
# KEY DIFFERENCE FROM _train2: T_MAX_TARGET_OH is no longer a fixed absolute
# time -- it's now min(data-availability ceiling, time-to-decay-to-a-fraction-
# of-OH's-own-peak), mirroring the H2O 95%-rise rule already applied in
# _train2. See the OH FRACTION-OF-PEAK RULE block below for the full reasoning
# and calibration against 1192 K.
#
...
# ~200 K hotter) than at 1192 K, where data ran out to ~1 ms. A target time picked
# past the last real data point doesn't error downstream -- np.interp() in the
# inference notebook silently CLIPS to the last available value, fabricating a flat
# "observation" at whatever time you picked past the data range. VERIFY the exact
# cutoff against hong_1398K_h2o.csv / hong_1398K_oh.csv (optimize_1398k_joint_nn.ipynb
# prints the true max time from each CSV) and tighten below if 0.1 ms is optimistic.
#
...
# T_MAX_TARGET_OH is NOT a fixed constant -- see the OH FRACTION-OF-PEAK RULE block
# right after the nominal OH profile is computed below. Two separate constraints
# apply, and we take whichever is MORE restrictive:
#   (1) DATA CEILING (hard, non-negotiable): never select a target past the last
#       time actually present in the experimental CSV -- exceeding it doesn't error,
#       it silently produces a wrong "observation" via np.interp's clip-to-boundary
#       behavior (see note above). This is a property of the DATASET, not the
#       kinetics, so it stays a fixed number here (tighten if the CSV's real range
#       turns out to be less than this).
#   (2) STRUCTURAL-VALIDITY CEILING (condition-adaptive): past some point in the
#       decay, missing radical-pool chemistry (OH+HO2 etc., not captured by the
#       2-reaction k1/k2 model) causes the model to diverge from reality, which
#       would bias k2 to compensate for chemistry it doesn't have. A rule tried
#       first -- "decay back to OH's initial ppm" -- turns out to be ill-posed: OH
#       is a transient radical intermediate, not a bounded reactant/product like
#       H2O, so it never returns anywhere near its initial value within any
#       practical window (checked directly: at 1192 K, OH is still ~14x its
#       initial value at t=1 ms). "Initial" itself isn't even a stable reference --
#       it's an artifact of grid resolution, not a physical quantity (at 1398 K,
#       OH_init is already ~20% of peak just because the kinetics are so fast).
#       PEAK, by contrast, is always well-defined (argmax always exists), so the
#       rule instead caps the window at the time OH decays to a FRACTION of its
#       OWN peak. Calibrated directly against 1192 K's working choice
#       (T_MAX_TARGET=3e-4, i.e. 0.3 ms): at that time OH sits at 71.5% of its
#       peak value. Applying that SAME 71.5%-of-peak fraction to 1398 K's own
#       profile gives t_max ~ 0.025 ms -- self-scaling, no per-temperature manual
#       re-tuning, and comfortably inside the 0.1 ms data ceiling so it doesn't
#       conflict with constraint (1).
T_MAX_TARGET_OH_DATA_CEILING = 1e-4    # 0.1 ms -- hard, dataset-derived (see above)
T_MAX_OH_PEAK_FRAC = 0.715             # calibrated from 1192 K; universal across
                                        # conditions -- only this fraction is tuned,
...
# T_MAX_TARGET_H2O is NOT a fixed constant either -- see the H2O-95%-RISE RULE block
# right after the nominal H2O profile is computed below. A fixed absolute cutoff
# (tried: 1e-4, inherited from the OH data ceiling) let independent D-optimality
# reach for the plateau (~100 us) as a 3rd target, which improved raw parameter
# identifiability but visibly cost early-rise fit accuracy -- the plateau point pulls
# the 2-parameter fit toward matching a late-time value at the expense of the
# early-rise shape you actually care about. A hardcoded smaller number (e.g. 5e-5)
# would fix THIS condition but isn't universal: the reaction timescale
# compresses/expands by ~10x between 1192 K and 1398 K, so any fixed millisecond
# value needs re-tuning by hand at every new temperature. Defining the cutoff as a
# FRACTION of each condition's own rise timescale (specifically: the time to reach
# 95% of the total H2O rise) self-scales automatically -- validated against 1192 K,
# where the working T_MAX=0.4ms almost exactly equals that condition's own
# 95%-rise time (~0.374 ms).
...
CHECKPOINT_PATH = 'ckpt_1398k_joint_train6.pt'
RESULT_PATH     = 'result_1398k_joint_train6.pt'
...
# ── OH FRACTION-OF-PEAK RULE: condition-adaptive T_MAX_TARGET_OH ──────────────
# See the long comment above for the full derivation. Find the first time AFTER
# the peak where OH has decayed to T_MAX_OH_PEAK_FRAC of its own peak value, then
# take min() against the hard data-availability ceiling.
_oh_decay_idx = np.where(_nom_oh[_i_pk:] <= T_MAX_OH_PEAK_FRAC * _oh_peak)[0]
if len(_oh_decay_idx) > 0:
    _t_oh_frac_cut = float(T_FINE[_i_pk + _oh_decay_idx[0]])
else:
    _t_oh_frac_cut = float(T_FINE[-1])   # never decays that far within the grid
T_MAX_TARGET_OH = min(T_MAX_TARGET_OH_DATA_CEILING, _t_oh_frac_cut)
print(f'  OH decays to {T_MAX_OH_PEAK_FRAC*100:.1f}% of peak at t={_t_oh_frac_cut*1e3:.4f} ms; '
      f'data ceiling={T_MAX_TARGET_OH_DATA_CEILING*1e3:.4f} ms '
      f'-> T_MAX_TARGET_OH={T_MAX_TARGET_OH*1e3:.4f} ms')
...
def select_targets_joint(t_oh, S_oh_mat, t_h2o, S_h2o_mat, n_oh, n_h2o, oh_min_ppm=30,
    ...
    Both species' t_max values passed in here are themselves computed by
    CONDITION-ADAPTIVE rules (fraction-of-own-peak for OH, fraction-of-own-rise
    for H2O) rather than fixed absolute times -- see the long comments above where
    T_MAX_TARGET_OH and T_MAX_TARGET_H2O are computed, right after each species'
    nominal profile is known.
    """
...
print(f'\nOH target probes (window {T_MIN_TARGET_OH*1e6:.1f} µs .. {T_MAX_TARGET_OH*1e3:.4f} ms, independent D-optimal):')
...
print(f'\nH2O target probes (window {T_MIN_TARGET_H2O*1e6:.1f} µs .. {T_MAX_TARGET_H2O*1e3:.4f} ms, independent D-optimal):')
...
print(f'\nTraining (1398 K joint OH+H2O, 3-pt each, OH: {T_MIN_TARGET_OH*1e6:.0f}us-{T_MAX_TARGET_OH*1e3:.4f}ms [peak-frac rule] / '
      f'H2O: {T_MIN_TARGET_H2O*1e6:.0f}us-{T_MAX_TARGET_H2O*1e3:.4f}ms [95%-rise rule]) ...')
t0 = time.time()
...
print(f'\n{"="*72}')
print('  1398 K Joint (OH + H2O) — 3-pt each (adaptive windows)  |  Zhang Table 1')
print(f'{"="*72}')
